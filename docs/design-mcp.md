...
| 連携解除後もアクセストークンが使われ続ける | 「連携中のアプリ」画面（6-2節）からの失効操作でリフレッシュトークン・同意記録を削除し、以降の更新を停止。既発行のアクセストークンは有効期限（既定7日、`accessTokenExpiresIn`）で自然失効させる。失効操作自体（リフレッシュトークン・同意記録の削除）はこの期限の長さに関わらず即座に効く |
...
確認ダイアログ必須（`docs/design-web.md`7章の基準に準拠）。実行ボタンは警告色（`destructive`バリアント）。失効操作は`oauth_consents`・`oauth_refresh_tokens`・`oauth_access_tokens`の該当行（ユーザー×クライアント単位）を削除する。既発行のアクセストークン（JWT）はステートレス検証のため即時失効はできず、有効期限（既定7日）による自然失効に委ねる（4-3節）。失効操作自体（リフレッシュトークンの削除による以降の更新停止、同意記録の削除）はこの期限の長さに関わらず即座に効く。
